...
* **Case-only name collisions (e.g. `Report.md` vs. `report.md`) are detected reliably only when this tool is actually run on Windows.** The check normalizes paths with Python's `os.path.normcase`, which folds case exclusively on Windows (this is how the Python standard library itself defines `normcase` — it is a documented no-op on every other OS, not a markitdown-specific limitation). Practically: on Linux, filenames differing only by case are genuinely different files on the usual case-sensitive filesystems, so this is a non-issue. On macOS, the default filesystem (APFS/HFS+) is normally case-insensitive despite this, so the *filesystem itself* still treats `Report.md` and `report.md` as the same file even though this pre-flight check won't report it as a collision — the practical effect is that one of the conversions silently determines the final content of that single file, without the batch aborting first. Avoid input folders with filenames that differ only by case if you rely on this tool on macOS.
